...
import xlong.blogs.service.LogsService;
...
public class PostController extends BaseController {
    ...
    public JsonResult pushPost(@ModelAttribute Post post, @RequestParam("cateList") List<String> cateList, @RequestParam("tagList") String tagList, HttpSession session) {
        ...
        try {
            //提取摘要
            int postSummary = 50;
            if (StringUtils.isNotEmpty(BlogConst.OPTIONS.get(BlogPropertiesEnum.POST_SUMMARY.getProp()))) {
                postSummary = Integer.parseInt(BlogConst.OPTIONS.get(BlogPropertiesEnum.POST_SUMMARY.getProp()));
            }
            //文章摘要
    ...
    public JsonResult pushAllToBaidu(@PathParam("baiduToken") String baiduToken) {
        ...
        String blogUrl = BlogConst.OPTIONS.get(BlogPropertiesEnum.BLOG_URL.getProp());
