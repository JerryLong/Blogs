...
import xlong.blogs.service.MailService;
import xlong.blogs.service.PostService;
import xlong.blogs.utils.OwoUtil;
...
public class CommentController extends BaseController {
    ...
    public String replyComment(@RequestParam("commentId") Long commentId,
        ...
        try {
            ...
            comment.setCommentAuthorUrl(BlogConst.OPTIONS.get(BlogPropertiesEnum.BLOG_URL.getProp()));
    ...
    class EmailToAuthor extends Thread {
        ...
        @Override
        public void run() {
            if (StringUtils.equals(BlogConst.OPTIONS.get(BlogPropertiesEnum.SMTP_EMAIL_ENABLE.getProp()), TrueFalseEnum.TRUE.getDesc()) && StringUtils.equals(BlogConst.OPTIONS.get(BlogPropertiesEnum.COMMENT_REPLY_NOTICE.getProp()), TrueFalseEnum.TRUE.getDesc())) {
                if (Validator.isEmail(lastComment.getCommentAuthorEmail())) {
                    Map<String, Object> map = new HashMap<>();
                    map.put("blogTitle", BlogConst.OPTIONS.get(BlogPropertiesEnum.BLOG_TITLE.getProp()));
                    map.put("commentAuthor", lastComment.getCommentAuthor());
                    map.put("pageName", lastComment.getPost().getPostTitle());
                    if (StringUtils.equals(post.getPostType(), PostTypeEnum.POST_TYPE_POST.getDesc())) {
                        map.put("pageUrl", BlogConst.OPTIONS.get(BlogPropertiesEnum.BLOG_URL.getProp()) + "/archives/" + post.getPostUrl() + "#comment-id-" + comment.getCommentId());
                    } else {
                        map.put("pageUrl", BlogConst.OPTIONS.get(BlogPropertiesEnum.BLOG_URL.getProp()) + "/p/" + post.getPostUrl() + "#comment-id-" + comment.getCommentId());
                    }
                    ...
                    map.put("blogUrl", BlogConst.OPTIONS.get(BlogPropertiesEnum.BLOG_URL.getProp()));
                    mailService.sendTemplateMail(
                            lastComment.getCommentAuthorEmail(), "您在" + BlogConst.OPTIONS.get(BlogPropertiesEnum.BLOG_URL.getProp()) + "的评论有了新回复", map, "common/mail/mail_reply.ftl");
                }
            }
        }
    }
                ...
                try {
                    if (status == 1 && Validator.isEmail(comment.getCommentAuthorEmail())) {
                        ...
                        mailService.sendTemplateMail(
                                ...
                                "您在" + BlogConst.OPTIONS.get(BlogPropertiesEnum.BLOG_URL.getProp()) + "的评论已审核通过！", map, "common/mail/mail_passed.ftl");
                    }
                } catch (Exception e) {
